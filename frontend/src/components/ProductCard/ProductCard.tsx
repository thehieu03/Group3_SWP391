...
const ProductCard: FC<{ product: ProductCardData; className?: string }> = ({
  product,
  className,
}) => {
  ...
  const handleCardClick = () => {
    navigate(routesConfig.getProductDetailsUrl(Number(product.id)));
  };

  return (
    <div
      onClick={handleCardClick}
      className={`rounded-lg border border-gray-100 bg-white p-3 shadow-sm transition hover:shadow cursor-pointer ${
        className ?? ""
      }`}
    >
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          <h3 className="line-clamp-2 text-[15px] font-bold text-emerald-700">
            {product.name}
          </h3>
          <div className="mt-1 flex items-center gap-2">
            <Stars value={product.rating} />
            <span className="text-xs text-gray-500">
              {product.reviews} Reviews | Đã bán:{" "}
              {product.sold.toLocaleString()} | Khiếu nại:{" "}
          ...
          <div className="mt-1 text-[13px]">
            ...
            <span onClick={(e) => e.stopPropagation()} className="inline-block">
              <Button
                to="#"
                className="ml-1 px-0 text-blue-600 hover:underline"
              >
                {product.seller}
              </Button>
            </span>
