...
// Tách các class chung ra constant
const MENU_ITEM_CLASS = 'h-full px-4 flex items-center text-white font-medium cursor-pointer hover:bg-emerald-600/90 transition-colors';
    ...
    return (
        <div className='h-full flex items-center text-white font-medium cursor-pointer transition-colors gap-2'>
            ...
            <Tippy content='Hỗ trợ' placement='bottom-end'>
                <Button to={routesConfig.support} className={MENU_ITEM_CLASS}>Hỗ trợ</Button>
            </Tippy>
            <Tippy content='Chia sẻ' placement='bottom-end'>
                <Button to={routesConfig.share} className={MENU_ITEM_CLASS}>Chia sẻ</Button>
            </Tippy>
            ...
            <Tippy content='FAQS' placement='bottom-end'>
                <Button to={routesConfig.faqs} className={MENU_ITEM_CLASS}>FAQS</Button>
            </Tippy>

            {isLogin && (
                <Button to={routesConfig.deposit} className={MENU_ITEM_CLASS}>Nạp tiền</Button>
            )}
        </div>
    );
}
