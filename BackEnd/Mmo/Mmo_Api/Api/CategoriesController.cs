...
public class CategoriesController : ControllerBase
{
    ...
    public CategoriesController(ICategoryServices categoryServices, ISubcategoryServices subcategoryServices,
        IMapper mapper)
    {
    ...
    public async Task<ActionResult<PaginationResponse<CategoryResponse>>> GetAllCategories([FromQuery] int? page = null,
        [FromQuery] int? pageSize = null, [FromQuery] bool? isActive = null)
    {
        try
        {
            var categories = await _categoryServices.GetAllAsync();
            if (!categories.Any()) return Ok(CreateEmptyPaginationResponse(pageSize ?? DefaultPageSize));
    ...
    public async Task<IActionResult> ActivateCategory(int id)
    {
        ...
        try
        {
            ...
            return result
                ? Ok(new { message = "Category activated successfully." })
                : BadRequest("Failed to activate category.");
        }
        catch (Exception)
        {
            return StatusCode(StatusCodes.Status500InternalServerError,
                "An error occurred while activating the category.");
        }
    }
    ...
    public async Task<IActionResult> UpdateCategory(int id, [FromBody] CategoryRequest categoryRequest)
    {
        ...
        try
        {
            ...
            if (result) return Ok(new { message = "Category updated successfully." });
